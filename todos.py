...
app.add_url_rule('/api/login/', view_func=login_view, methods=['POST', 'OPTIONS'])
rgs_view = RegisterAPI.as_view('rgs_api')
app.add_url_rule('/api/register/', view_func=rgs_view, methods=['POST','OPTIONS'])

#todos
note_view = NoteAPI.as_view('note_api')
app.add_url_rule('/api/todos/', view_func=note_view, methods=['GET','POST','PUT', 'OPTIONS'])

#delete todos
delete_view = DeleteAPI.as_view('delete_api')
app.add_url_rule('/api/delete/', view_func=delete_view, methods=['PUT', 'OPTIONS'])

#all todos completed
allcompeleted_view = AllCompleteAPI.as_view('allcompeleted_api')
app.add_url_rule('/api/compeleted/', view_func=allcompeleted_view, methods=['PUT', 'OPTIONS'])
